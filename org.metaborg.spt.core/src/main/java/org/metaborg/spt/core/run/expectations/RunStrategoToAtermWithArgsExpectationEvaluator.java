...
public class RunStrategoToAtermWithArgsExpectationEvaluator
    implements ISpoofaxExpectationEvaluator<RunStrategoToAtermWithArgsExpectation> {
    private static final ILogger logger = LoggerUtils.logger(RunStrategoToAtermWithArgsExpectationEvaluator.class);
    ...
    @Inject public RunStrategoToAtermWithArgsExpectationEvaluator(IContextService contextService,
    ...
    @Override public Collection<Integer> usesSelections(IFragment fragment, RunStrategoToAtermWithArgsExpectation expectation) {
        return expectation.selection() == null ? Lists.<Integer>newArrayList()
            : Lists.newArrayList(expectation.selection());
    }

    @Override public TestPhase getPhase(ILanguageImpl language, RunStrategoToAtermWithArgsExpectation expectation) {
    ...
    @Override public ISpoofaxTestExpectationOutput evaluate(
        ...
        RunStrategoToAtermWithArgsExpectation expectation) {
        ...
        for(IStrategoTerm term : terms) {
            ...
            try {
                final IStrategoTerm result =
                    context == null ? stratego.invoke(input.getLanguageUnderTest(), test.getResource(), term, strategy, arguments)
                        : stratego.invoke(input.getLanguageUnderTest(), context, term, strategy, arguments);
     ...
     */
    private List<IStrategoTerm> runOnTerms(ITestCase test, RunStrategoToAtermWithArgsExpectation expectation,
